...
__global__ void row_rmsnorm_f32_dim_simd(float* in, float* wei, float* out,
  ...
  // 线程内平方和，利用float4的SIMD特性一次处理4个元素
  ...
  // 从 pack_off 到 size - 1 的那几个元素，是不能凑满 4 个的一小段尾巴
...
// 一个block处理一行，每个线程处理size/blockDim.x个元素
